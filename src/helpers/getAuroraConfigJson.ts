...
const validateConfigurationObject = (config: AuroraConfig) => {
  ...
  if ( config.files.filter( file => !file.includes('.prisma')).length ) {
    console.error(
      `Invalid File. Only provide paths to .prisma files.`
    );
  ...
  if (!config?.output?.length) {
    console.error(
      `No Output location. Please specify where to output the generated file.`
    );
  ...
  if ( !config.output.includes('.prisma') ) {
    console.error(
      `The Output file should have the .prisma extension.`
    );
    throw new Error(ERRORS.NON_PRISMA_FILE);
  }
};
