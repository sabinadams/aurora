...
export default function (datamodel: string): { [K: string]: IndexObject[] } {
  ...
  return modelChunks.reduce((modelDefinitions: { [k: string]: any }, modelChunk: string) => {
    ...
    const fieldsWithIndexes = pieces
      .slice(1)
    ...
    modelDefinitions[pieces[0].split(' ')[1]] = fieldsWithIndexes.reduce(
      (indexes: IndexObject[], field: string) => [
        ...indexes,
        {
          ...
          fields: field
            ...
            .split(']')[0]
            .split(',')
            .map((indexField) => indexField.trim())
        }
      ],
      []
    );
