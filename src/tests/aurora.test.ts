    ...
    describe('Model Blocks', () => {
        ...
        describe('Model Fields', () => {
            ...
            it('should render ?', async () => {
                const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-?.prisma'])
            ...
            describe('@id', () => {
                it('should render @id', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@id.prisma'])
                    expect(generatedSchema).toContain('id Int @id')
                })

                it('should render @id(map: "")', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@id-map.prisma'])
                    expect(generatedSchema).toContain('id Int @id(map: "test")')
                })

                it('should render @id(length: 1)', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@id-length.prisma'])
                    expect(generatedSchema).toContain('id String @id(length: 1)')
                })

                it('should render @id(sort: Asc)', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@id-sort.prisma'])
            ...
            describe('@default', () => {
                it('should render @default("")', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@default.prisma'])
                    expect(generatedSchema).toContain('id String @id @default("test")')
                })

                it('should render @default(value: "")', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@default-value.prisma'])
                    expect(generatedSchema).toContain('id String @id @default(value: "test")')
                })

                it('should render @default(map: "", "")', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@default-map.prisma'])
            ...
            describe('@unique', () => {
                it('should render @unique', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@unique.prisma'])
                    expect(generatedSchema).toContain('id Int @id @unique')
                })

                it('should render @unique(map: "")', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@unique-map.prisma'])
                    expect(generatedSchema).toContain('id Int @id @unique(map: "mapping")')
                })

                it('should render @unique(length: 1)', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@unique-length.prisma'])
                    expect(generatedSchema).toContain('id String @unique(length: 1)')
                })

                it('should render @unique(sort: Asc)', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@unique-sort.prisma'])
            ...
            describe('@relation', () => {
                it('should render @relation("")', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@relation.prisma'])
                ...
                it('should render @relation(name: "", fields: [], references: [])', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@relation-name.prisma'])
                ...
                it('should render @relation(fields: [], references: [])', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@relation-unnamed.prisma'])
                    expect(generatedSchema).toContain('@relation(fields: [id], references: [id])')
                })

                it('should render @relation(onDelete: action)', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@relation-ondelete.prisma'])
                    expect(generatedSchema).toContain('@relation(fields: [id], references: [id], onDelete: Cascade)')
                })

                it('should render @relation(onUpdate: action)', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@relation-onupdate.prisma'])
                    expect(generatedSchema).toContain('@relation(fields: [id], references: [id], onUpdate: Cascade)')
                })

                it('should render @relation(map: "")', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@relation-map.prisma'])
            ...
            describe('@map', () => {
                it('should render @map("")', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@map.prisma'])
                    expect(generatedSchema).toContain('id String @id @map("test")')
                })

                it('should render @map(name: "")', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@map-name.prisma'])
            ...
            describe('@updatedAt', () => {
                it('should render @updatedAt', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@updatedAt.prisma'])
            ...
            describe('@ignore', () => {
                it('should NOT render @ignore', async () => {
                    const generatedSchema = await getGeneratedSchema(['feature-specific/model-fields/model-field-@ignore.prisma'])
